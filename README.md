...
[![License](https://img.shields.io/badge/license-PolyForm%20Noncommercial-14171C?style=flat-square)](LICENSE)
...
Download `slate-<version>-full.bin` from
[Releases](../../releases) and write it at offset `0x0`:

```bash
esptool.py --chip esp32c3 -p /dev/cu.usbmodemXXXX write_flash 0x0 slate-v1.0.0-beta1-full.bin
```

The Supermini appears as `/dev/cu.usbmodem*` on macOS and `/dev/ttyACM*` on
Linux — **not** `/dev/ttyUSB*`. If no port shows up, hold BOOT, tap RESET,
release BOOT, and try again.

Every release also has a `-update.bin`. That one is for the settings page and
nothing else — writing it at `0x0` will not boot.
...
**[PolyForm Noncommercial 1.0.0](LICENSE).** Read it, run it, change it, share
it — for any noncommercial purpose. Hobby projects, personal builds, study and
experiment are all expressly permitted.

**Commercial use is not.** You may not sell this software, sell hardware built
on it, or use it for commercial advantage.
